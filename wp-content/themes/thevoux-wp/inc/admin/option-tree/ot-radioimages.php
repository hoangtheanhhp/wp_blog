...
function thb_filter_typography_fields2( $array, $field_id ) {
	...
	if ( in_array($field_id, array("title_type", "body_type", "button_type", "em_font") ) ) {
		$array = array( 'font-family');
	}
	if ( in_array($field_id, array("em_font") ) ) {
	...
	if ( in_array($field_id, array("menu_type", "submenu_type", "subheader_menu_type","mobile_menu_type", "mobile_submenu_type", "post_meta_type", "social_bar_type", "footer_menu_type") ) ) {
...
function thb_clear_font_cache() {
	$clear = isset($_GET['thb_clear_font_cache']) ? $_GET['thb_clear_font_cache'] : false;
