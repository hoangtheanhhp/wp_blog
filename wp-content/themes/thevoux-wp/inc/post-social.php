...
function thb_fbLikeCount($pageID, $debug = false) {
  if ( ot_get_option('facebook_app_id') == '416706622025229' && $_SERVER['HTTP_HOST'] !== 'thevoux.fuelthemes.net') {
    return esc_html_e('Please enter your Facebook App ID inside Theme Options', 'thevoux');
  }
	...
	if ( empty( $cache ) ) {
		...
		$json_output = json_decode($data);
	 	...
	 	if (isset($json_output->error)) {
	 	  echo esc_html($json_output->error->message);
		...
		$likes = isset($json_output->fan_count) ? $json_output->fan_count : 0;
...
function thb_getTwitterFollowers($debug = false) {
  if ( ot_get_option('twitter_bar_consumerkey') == 't0w3HYVODhFG0m34IFXg' && $_SERVER['HTTP_HOST'] !== 'thevoux.fuelthemes.net') {
    return esc_html_e('Please enter your Twitter App settings inside Theme Options', 'thevoux');
  }
  $settings = array(
...
function thb_getInstagramFollowers() {
  ...
  if ( $api_key == '1671492.443336d.3493a6f64ae34f6dae4afcf692d46cd8' && $_SERVER['HTTP_HOST'] !== 'thevoux.fuelthemes.net') {
    return esc_html_e('Please enter your Instagram API key inside Theme Options', 'thevoux');
  }
...
function thb_social_article_detail($id = false, $fixed = false, $class = false) {
  ...
  $twitter_user = $twitter_user ? $twitter_user : get_bloginfo( 'name' );
 ...
 ?>
	<aside class="<?php echo esc_attr(implode(' ', $classes)); ?>">
		<?php if (in_array('facebook',$sharing_type)) { ?>
  		<a href="<?php echo 'http://www.facebook.com/sharer.php?u=' . urlencode( esc_url( $permalink ) ).''; ?>" class="boxed-icon facebook social <?php echo esc_attr($sharing_style); ?>"><i class="fa fa-facebook"></i>
		...
		<?php } ?>
		<?php if (in_array('twitter',$sharing_type)) { ?>
		<a href="<?php echo 'https://twitter.com/intent/tweet?text=' . htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . '&url=' . urlencode( esc_url( $permalink ) ) . '&via=' . urlencode( $twitter_user ); ?>" class="boxed-icon twitter social <?php echo esc_attr($sharing_style);?>"><i class="fa fa-twitter"></i>
		...
		<?php } ?>
		<?php if (in_array('google-plus',$sharing_type)) { ?>
		<a href="<?php echo 'http://plus.google.com/share?url=' . esc_url( $permalink ) . ''; ?>" class="boxed-icon google-plus social <?php echo esc_attr($sharing_style);?>"><i class="fa fa-google-plus"></i>
			<?php if ($hide_zero_shares === 'off' || pssc_gplus($id) !== '0') { ?>
			<span><?php echo thb_numberAbbreviation(pssc_gplus($id)); ?></span>
			<?php } ?>
		<?php } ?>
		<?php if (in_array('pinterest',$sharing_type)) { ?>
		<a href="<?php echo 'http://pinterest.com/pin/create/link/?url=' . esc_url( $permalink ) . '&media=' . ( ! empty( $image[0] ) ? $image[0] : '' ) . '&description='.htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8'); ?>" class="boxed-icon pinterest social <?php echo esc_attr($sharing_style);?>" data-pin-no-hover="true"><i class="fa fa-pinterest"></i>
			<?php if ($hide_zero_shares === 'off' || pssc_pinterest($id) !== '0') { ?>
			<span><?php echo thb_numberAbbreviation(pssc_pinterest($id)); ?></span>
			<?php } ?>
		<?php } ?>
		<?php if (in_array('linkedin',$sharing_type)) { ?>
		<a href="<?php echo 'https://www.linkedin.com/cws/share?url=' . esc_url( $permalink ) . ''; ?>" class="boxed-icon linkedin social <?php echo esc_attr($sharing_style);?>"><i class="fa fa-linkedin"></i>
			<?php if ($hide_zero_shares === 'off' || pssc_linkedin($id) !== '0') { ?>
			<span><?php echo thb_numberAbbreviation(pssc_linkedin($id)); ?></span>
			<?php } ?>
		</a>
		<?php } ?>
    <?php $comment_icon = $sharing_style == 'style1' ? 'comment' : 'comment-style2'; ?>
		<a href="<?php the_permalink(); ?>" class="boxed-icon comment <?php echo esc_attr($sharing_style);?>"><?php get_template_part('assets/svg/'.$comment_icon.'.svg'); ?><span><?php echo get_comments_number(); ?></span></a>
	</aside>
<?php
}
...
function thb_social_product($id = false) {
 ...
 <aside class="share-article">
 	<?php if (in_array('facebook',$sharing_type)) { ?>
 	<a href="<?php echo 'http://www.facebook.com/sharer.php?u=' . urlencode( esc_url( $permalink ) ).''; ?>" class="boxed-icon facebook social <?php echo esc_attr($sharing_style); ?>"><i class="fa fa-facebook"></i><span><?php echo thb_numberAbbreviation(pssc_facebook($id)); ?></span></a>
 	<?php } ?>
 	<?php if (in_array('twitter',$sharing_type)) { ?>
 	<a href="<?php echo 'https://twitter.com/intent/tweet?text=' . htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . '&url=' . urlencode( esc_url( $permalink ) ) . '&via=' . urlencode( $twitter_user ? $twitter_user : get_bloginfo( 'name' ) ) . ''; ?>" class="boxed-icon twitter social <?php echo esc_attr($sharing_style); ?>"><i class="fa fa-twitter"></i><span><?php echo thb_numberAbbreviation(pssc_twitter($id)); ?></span></a>
 	<?php } ?>
 	<?php if (in_array('google-plus',$sharing_type)) { ?>
 	<a href="<?php echo 'http://plus.google.com/share?url=' . esc_url( $permalink ) . ''; ?>" class="boxed-icon google-plus social <?php echo esc_attr($sharing_style); ?>"><i class="fa fa-google-plus"></i><span><?php echo thb_numberAbbreviation(pssc_gplus($id)); ?></span></a>
 	<?php } ?>
 	<?php if (in_array('pinterest',$sharing_type)) { ?>
 	<a href="<?php echo 'http://pinterest.com/pin/create/link/?url=' . esc_url( $permalink ) . '&amp;media=' . ( ! empty( $image[0] ) ? $image[0] : '' ) . ''; ?>" class="boxed-icon pinterest social <?php echo esc_attr($sharing_style); ?>" nopin="nopin" data-pin-no-hover="true"><i class="fa fa-pinterest"></i><span><?php echo thb_numberAbbreviation(pssc_pinterest($id)); ?></span></a>
 	<?php } ?>
 </aside>
 <?php } ?>
<?php
